...
<div class="container">
    ...
    <div class="grid grid-flow-col gap-5">
        <a href="/admin/users"
            class="block max-w-xs py-8 ps-8 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100">
            <div class="flex gap-5">
                ...
                <div class="flex flex-col">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">{{ count($users) ?? '0' }}
                    </h5>
        ...
        <a href="#"
            ...
            <div class="flex gap-5">
                ...
                <div class="flex flex-col">
                    ...
                    <p class="font-normal text-gray-700 text-wrap">Can't create User</p>
                </div>
            </div>
        </a>
        <a href="/admin/users"
            class="block max-w-xs py-8 ps-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100">
            <div class="flex gap-5">
                ...
                <div class="flex flex-col">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 ">Total verified
                    </h5>
                    <p class="font-normal text-gray-700 ">{{ $userVerifiedCount }}</p>
                </div>
            </div>
        </a>
        <a href="/admin/users/deleted"
            class="block max-w-xs py-8 ps-8 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100">
            <div class="flex gap-5">
            ...
            <tbody>
                ...
                <tr class="bg-white border-bhover:bg-gray-50 text-zinc-900">
                    ...
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        <a href="{{ $data->image_url }}" class="flex gap-2">
                            <img src="{{ $data->image_url ?? asset('img/pic.png')  }}" alt="pic"
                                class="w-10 h-10 rounded-sm object-cover">
                            {{ $data->name ?? 'unknown' }}
                        </a>
                    </th>
                    <td class="px-6 py-4 font-semibold">
                        {{ $data->address ?? 'Belum Ada' }}
                    </td>
                    <td class="px-6 py-4 font-semibold">
                        {{ $data->phone ?? 'Belum Ada' }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $data->email }}
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <p
                            class="font-medium {{ empty($data->email_verified_at) ? 'text-red-600 bg-red-200' : 'text-green-600 bg-green-200'}} p-1 rounded-xl flex items-center justify-center">
                            {{
                            empty($data->email_verified_at) ? 'Inactive' : 'Active'
                            }}</p>
                    </td>
                    <td class="px-6 py-4 text-right flex gap-4">
                        <a href="users/edit/{{ $data->id }}" class="font-medium text-blue-600 hover:underline">Edit</a>

                        <form action="users/delete/{{$data->id}}" method="POST" class="inline">
                            @csrf
                            @method('delete')
                            <button type="submit" class="font-medium text-red-600 hover:underline"
                                onclick="return confirm('Are you sure?') ? window.location.href='users/delete/{{$data->id}}' : false">Delete</button>
                        </form>
                    </td>
                </tr>
                @endforeach
